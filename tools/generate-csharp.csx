...
// ProtocolCommand enum
{
    ...
    sb.AppendLine();
    sb.AppendLine("/// <summary>Protocol department IDs. Full opcode = (DepartmentId &lt;&lt; 10) | OpcodeValue.</summary>");
    ...
    Dictionary<string, (string DeptPascal, string DeptHex, int OpcodeValue)> opcodes)
{
    ...
    foreach (var (structName, sizeOf, rawFields) in defs)
    {
        ...
        if (hasOpcode)
        {
            var fullOpcode = (int.Parse(opcodeInfo.DeptHex.Replace("0x", ""), System.Globalization.NumberStyles.HexNumber) << 10) | opcodeInfo.OpcodeValue;
...

{
    ...
    {
        var fullOpcode = (int.Parse(e.deptHex.Replace("0x", ""), System.Globalization.NumberStyles.HexNumber) << 10) | e.opcodeValue;
