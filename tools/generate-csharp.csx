...
void WriteClassFile(string filePath, string ns, IEnumerable<(string name, int sizeOf, JsonElement fields)> defs)
{
    ...
    foreach (var (structName, sizeOf, rawFields) in defs)
    {
        ...
        // Stream overloads
        sb.AppendLine("    public void Read(Stream stream) => Read(new BinaryReader(stream, System.Text.Encoding.UTF8, true));");
        sb.AppendLine("    public void Write(Stream stream) => Write(new BinaryWriter(stream, System.Text.Encoding.UTF8, true));");
